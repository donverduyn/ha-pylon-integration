...
class PylontechBatteryEntity(CoordinatorEntity[PylontechCoordinator]):
    ...
    def __init__(self, coordinator, entry_id: str, bat_id: int):
        super().__init__(coordinator)
        self._entry_id = entry_id
        self._bat_id = bat_id
    ...
    def device_info(self) -> DeviceInfo:
        data = self.coordinator.data
        return DeviceInfo(
            identifiers={(DOMAIN, f"{self._entry_id}_battery_{self._bat_id}")},
            ...
            model=data.get("model") if data else None,
            sw_version=data.get("fw_version") if data else None,
            via_device=(DOMAIN, f"{self._entry_id}_system"),
        )
...
class PylontechCellEntity(PylontechBatteryEntity):
    ...
    def __init__(self, coordinator, entry_id: str, bat_id: int, cell_id: int):
